...
api.get('/test', function(req, res) {
    console.log('TEST called ' + req.body.test);
...
api.get('/chatDetails', function(req, res) {
    ...
    Util.getChatDetails(userId, guestId, res);
})
                ...
                if(err) {
                    ...
                    Connection.findOneAndUpdate({userId : selectedConnection.userId}, { $push : {guests : guestId}}, function(err, doc) {
                        if(err) {
                            ...
                            res.json({success : true, message : 'You are now connected to ' + userWithMinConnection.name, user : userWithMinConnection, guest : guest});
                        }
                    });



                    /*User.findOne({_id : selectedConnection.userId}, function(err, user) {
                        if(err) {
                            res.status(500).json({message : 'Guest Activation : Finding selected user.'})
                        }
                        else {
                            var guest = {
                                _id :  randomString(10) + new Date().getTime() + randomString(10) + '_JUVENIK',
                                name : "Guest " + (selectedConnection.count+1),
                                pic : "/guest.jpg"
                            };

                            res.json({success : true, message : 'You are now connected to ' + user.name, user : user, guest : guest});
                        }
                    })*/
                }
            })
